...
// ════════════════════════════════════════════════════════════════════════════
// Bible Service
// English: bible-api.com (KJV, WEB, BBE, OEB)
// Telugu:  bolls.life API with fallback
// ════════════════════════════════════════════════════════════════════════════
...
class SearchResult {
  ...
  const SearchResult({
    ...
    required this.verse, required this.text,
  });
...
class BibleService {
  ...
  Future<BibleChapter> _fetchEnglish({
    ...
    if (verses.isEmpty) throw Exception('No verses found for $bookName $chapter');
  ...
  // ── Telugu API — bolls.life with proper error handling ────────────────
  Future<BibleChapter> _fetchTelugu({
    required String bookName,
    required int    chapter,
  }) async {
    final bookId = _teluguBookId(bookName);
    if (bookId == null) throw Exception('$bookName not found in Telugu Bible');
          ...
          if (data.isNotEmpty) {
            final verses = data.asMap().entries.map((e) {
              ...
              if (v is Map<String, dynamic>) {
                verseNum = (v['verse'] as num?)?.toInt() ?? (e.key + 1);
              ...
              return BibleVerse(verse: verseNum, text: text);
    ...
    // ── Fallback: fetch English KJV and note ─────────────────────────────
    throw Exception(
      'తెలుగు బైబిల్ API అందుబాటులో లేదు.\n'
      'Telugu Bible API unavailable. Please check internet connection.');
  }
  ...
  Future<BibleVerse> fetchDailyVerse(String translation) async {
    ...
    try {
      ...
      if (resp.statusCode == 200) {
        ...
        if (text.isNotEmpty) {
          return BibleVerse(verse: 1, text: text);
        }
      }
    } catch (_) {}
  ...
  Future<List<SearchResult>> search(String query, String translation) async {
    ...
    try {
      ...
      return (data['verses'] as List).map((v) => SearchResult(
